            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="card-box" id="uploadhide">
                        <form method="post" action="{{ url('admin/storescrubfile') }}" class="form-validate-summernote" id="myform" enctype="multipart/form-data">
                            @csrf
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                <label class="label label-{{ $export->status == 'processed' ? 'success' : ($export->status == 'processing' ? 'info' : 'danger') }}">{{ ucwords($export->status) }}</label>
                                            </td>
                                            <td>
                                            ...
                                            <td class="text-center">
                                                ...
                                                <span style="display:flex; justify-content: space-evenly;">
                                                    <a class="btn btn-success" href="{{ $export->status == 'processed' ? asset('dnc-seperated/' . $export->paths['combined']) : '' }}">DNC/NonDnc</a>

                                                </span>

                                                @else
                                                <span style="display:flex; justify-content: space-evenly;">
                                                    <a class="btn btn-success" href="{{ $export->status == 'processed' ? asset('dnc-seperated/' . $export->paths['dnc']) : '' }}">DNC</a>
                                                    <a class="btn btn-danger" href="{{ $export->status == 'processed' ? asset('dnc-seperated/' . $export->paths['non_dnc']) : '' }}">NON-DNC</a>


                                                    @if(!empty($export->invalid_dnc_count))
                                                    <a class="btn btn-warning" href="{{ $export->status == 'processed' ? asset('dnc-seperated/' . $export->paths['invalid']) : '' }}">INVALID</a>
                                                    @endif
                                                </span>
                                            ...
                                            <td>
                                                <a href="{{ url('admin/pdf/' . $export->id . '') }}" class="btn btn-sm btn-success" target="_blank">
                                                    Genrate Pdf
                                                </a>
                                            </td>
                                        </tr>
    ...
    $(document).on('click', '.modalBtn', function() {
        ...
        for (i in content) {
            ...
            if (content[i].invalid !== undefined) {
                html += `<a class="btn btn-warning" href="{{ asset('dnc-seperated/${content[i]?.invalid}') }}">INVALID-DNC</a></div></div>`;
            }
        }
        html += '</div>';
