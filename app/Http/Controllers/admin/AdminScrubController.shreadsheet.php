...
class AdminScrubController extends Controller
{
	...
	public function __construct(private SpreadsheetService $spreadsheetService)
	{
	}
	...
	public function seperateList(Request $request)
	{
		...
		try {
			...
			if ($request->option === 'combined') {
				...
				$data['filenames'][] = [
					'name' => time() . '_combnied' . '.xlsx',
					'sheets' => ['dnc', 'nondnc', 'invalid'],
					'headers' => [
						'dnc' => ['phone', 'federal', 'litigator', 'internal', 'wireless'],
						'nondnc' => ['phone'],
						'invalid' => ['phone'],
					],
				];
				...
				if (isset($data['filenames'])) {
					foreach ($data['filenames'] as $index => $filename) {
						...
						$this->spreadsheetService->setFilePath($filePath)
							->setSheetNames($filename['sheets'])
							->setHeaders($filename['headers'])
							->create()
							->writeHeader();
					}
				}
					...
					foreach ($data['filenames'] as $index => $filename) {
						...
						if (isset($filename['type']) && $filename['type'] === 'dnc') {
							...
							foreach ($regions as $id => $region) {
								$region = str_replace(' ', '_', $region);
								$sheets[] = $region;
								$headers[$region] = ['phone', 'federal', 'litigator', 'internal', 'wireless'];
							}

							$this->spreadsheetService = $this->spreadsheetService->setHeaders($headers);
							$this->spreadsheetService = $this->spreadsheetService->setSheetNames($sheets)
								->create()
								->writeHeader();
			...
			$dncExport = $dncExport->create([
				'name' => $request->name,
				'paths' => json_encode($data['filenames']),
				'user_id' => auth()->user()->id,
			]);
			$data['export_id'] = $dncExport->id;
